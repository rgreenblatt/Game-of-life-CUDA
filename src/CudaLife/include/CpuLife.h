...
namespace mf {
	...
	public:
		...
		bool allocBuffers(bool bitLife) {
			...
			try {
				...
				m_bpcData = new uint8_t[bitDataLength];

				if (bitLife) {
					m_dataLength = bitDataLength;
					m_bpcResultData = new uint8_t[m_dataLength];
				}
				else {
					m_dataLength = dataLength;
					m_data = new uint8_t[m_dataLength];
					m_resultData = new uint8_t[m_dataLength];
				}
			}
			catch (std::bad_alloc&) {
				freeBuffers();
			...
			for (size_t i = 0; i < iterations; ++i) {
				for (size_t y = 0; y < m_worldHeight; ++y) {
					...
					for (size_t x = 0; x < m_worldWidth; ++x) {
						...
						uint8_t aliveCells = countAliveCells(m_data, x0, x, x2, y0, y1, y2);
		...
		bool iterateParallelLambda(size_t iterations) {
			auto evaluateCell = [&] (size_t index) {
				...
				uint8_t aliveCells = countAliveCells(m_data, x0, x1, x2, y0, y1, y2);
		...
		bool iterateBitPerPixelCountingParallelLambda(size_t iterations, size_t blocksPerThread, size_t worldDataWidth,
			...
			auto evaluateCell = [worldDataWidth, worldSize, blocksPerThread, this] (size_t index) {
				...
				for (uint i = 0; i < blocksPerThread; ++i) {
					...
					m_bpcResultData[oldX + yAbs] = uint8_t(result);
				}
			};
	...
	private:
		...
		static inline uint8_t countAliveCells(uint8_t* data, size_t x0, size_t x1, size_t x2, size_t y0, size_t y1,
				size_t y2) {
		...
		static void iterateParallelStep(size_t index) {
			...
			uint8_t aliveCells = countAliveCells(s_data, x0, x1, x2, y0, y1, y2);
		...
		void precompute6x3EvaluationTable() {
			...
			m_lookupTable = new uint8_t[1 << 18];
			Concurrency::parallel_for<size_t>(0, 1 << 18, 1, evaluateTable);
		}

	};

	template <typename T> uint8_t* TCpuLife<T>::s_data;
	template <typename T> uint8_t* TCpuLife<T>::s_resultData;
