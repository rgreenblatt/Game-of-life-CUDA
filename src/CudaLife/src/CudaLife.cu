...
namespace mf {
	...
	__global__ void simpleLifeKernel(const uint8_t* lifeData, uint worldWidth, uint worldHeight, uint8_t* resultLifeData) {
	...
	void runBitLifeEncodeKernel(const uint8_t* d_lifeData, uint worldWidth, uint worldHeight, uint8_t* d_encodedLife) {
	...
	__global__ void precompute6x3EvaluationTableKernel(uint8_t* resultEvalTableData) {
	...
	void runPrecompute6x3EvaluationTableKernel(uint8_t* d_lookupTable) {
	...
	/// @param resultLifeData  Result buffer in the same format as input.
	__global__ void bitLifeKernelLookup(const uint8_t* lifeData, uint worldDataWidth, uint worldHeight,
			uint bytesPerThread, const uint8_t* evalTableData, uint8_t* resultLifeData) {
	...
	bool runBitLifeKernel(uint8_t*& d_encodedLifeData, uint8_t*& d_encodedlifeDataBuffer, const uint8_t* d_lookupTable,
		...
		size_t worldEncDataWidth = worldWidth / 8;
		if (d_lookupTable == NULL && useBigChunks) {
			size_t factor = sizeof(uint) / sizeof(uint8_t);
			if (factor != 4) {
				return false;
			}
